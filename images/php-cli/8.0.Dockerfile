ARG IMAGE_REPO
ARG IMAGE_TAG
FROM ${IMAGE_REPO:-lagoon}/php-8.0-fpm:${IMAGE_TAG:-latest}

ENV LAGOON=cli

RUN curl -L -o /usr/local/bin/composer https://github.com/composer/composer/releases/download/2.4.3/composer.phar \
